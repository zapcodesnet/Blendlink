...
# MongoDB connection - connects to your dedicated BlendLink cluster
def get_mongo_connection():
    """
    Connects to MongoDB Atlas (dedicated BlendLink cluster).
    Supports two config styles:
      1. MONGODB_URI (full connection string with db name) - recommended for Render
      2. MONGO_URL + DB_NAME (split) - legacy support
    Falls back to local MongoDB for development.
    """
    # Style 1: Single URI (recommended for Render deployment)
    mongodb_uri = os.environ.get('MONGODB_URI')
    if mongodb_uri:
        # Extract db name from URI if present (after last / before ?)
        from urllib.parse import urlparse
        parsed = urlparse(mongodb_uri)
        db_name = parsed.path.strip('/') or os.environ.get('DB_NAME', 'blendlink_db')
        # Build clean URL without db name for motor
        mongo_url = mongodb_uri
        print(f"📌 Using MONGODB_URI connection style")
    else:
        # Style 2: Split config (legacy)
        mongo_url = os.environ.get('MONGO_URL')
        db_name = os.environ.get('DB_NAME', 'blendlink_db')
...
async def register(data: UserCreate, response: Response):
    ...
    banned = await db.banned_emails.find_one({"email": normalized_email})
    if banned:
...
# REMOVED: verify-test-user endpoint contained hardcoded credentials
# This was a security risk - use proper test frameworks instead
...
async def google_session(request: Request, response: Response):
    ...
    async with httpx.AsyncClient() as client_http:
        # OAuth session endpoint - configure via GOOGLE_OAUTH_SESSION_URL env var
        oauth_url = os.environ.get('GOOGLE_OAUTH_SESSION_URL', 'https://demobackend.emergentagent.com/auth/v1/env/oauth/session-data')
...
async def get_user(user_id: str, request: Request):
    ...
    # PUBLIC FIELDS ONLY for non-self viewers to prevent data exposure
    PUBLIC_FIELDS = {
        "user_id", "username", "display_name", "name_hidden", "avatar",
        "profile_picture", "profile_picture_mint_id", "bio",
        "follower_count", "following_count", "post_count",
        "is_premium", "is_verified", "created_at", "name",
        "medal_showcase", "badges", "level", "xp"
    }
    if not is_self:
        user = {k: v for k, v in user.items() if k in PUBLIC_FIELDS}
...
async def play_spin_wheel(current_user: dict = Depends(get_current_user)):
    ...
    result = await db.users.find_one_and_update(
        {"user_id": current_user["user_id"], "bl_coins": {"$gte": bet}},
        {"$inc": {"bl_coins": -bet}},
    ...
    rand = secrets.randbelow(total_weight) + 1
    cumulative = 0
    result = outcomes[0]
...
async def play_scratch_card(current_user: dict = Depends(get_current_user)):
    ...
    card = [[_secure_random.choice(symbols) for _ in range(3)] for _ in range(3)]
...
async def upload_file(
    ...
    # Read file content with size check during read
